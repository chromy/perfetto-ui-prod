var perfetto = (function (exports) {
	...
	let gWarmWasmWorker = null;
	function createNewWasmEngineWorker() {
	    return new Worker('engine_bundle.js');
	}
	function createWasmEngineWorker() {
	 ...
	 */
	function warmupWasmEngineWorker() {
	    if (gWarmWasmWorker !== null) {
	        return;
	    }
	    gWarmWasmWorker = createNewWasmEngineWorker();
	}
	exports.warmupWasmEngineWorker = warmupWasmEngineWorker;
	/**
	...
	unwrapExports(canvas_controller);
	var canvas_controller_1 = canvas_controller.CanvasController;

	var canvas_wrapper = createCommonjsModule(function (module, exports) {
	// Copyright (C) 2018 The Android Open Source Project
	//
	// Licensed under the Apache License, Version 2.0 (the "License");
	// you may not use this file except in compliance with the License.
	...
	***************************************************************************** */
	...
	function doQuery(engine, query) {
	    return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	        ...
	        try {
	            const result = yield engine.rawQuery({
	                sqlQuery: query,
	            });
	            console.log(result);
	...
	function table(result) {
	    if (!result)
	        return mithril('');
	    const extract = (d, i) => {
	        if (!d || !d.longValues || !d.doubleValues || !d.stringValues)
	            return 0;
	        if (d.longValues.length > 0)
	            return +d.longValues[i];
	        if (d.doubleValues.length > 0)
	            return +d.doubleValues[i];
	        if (d.stringValues.length > 0)
	            return d.stringValues[i];
	        return 0;
	    };
	...
	const QueryBox = {
	    view() {
	        ...
	        for (let i = 0; i < ExampleQueries.length; i++) {
	            if (i !== 0)
	                examples.push(', ');
	            examples.push(mithril(ExampleQuery, { chosen: () => this.query = ExampleQueries[i].query }, ExampleQueries[i].name));
	        }
	        return mithril('form', {
	            onsubmit: (e) => {
	                e.preventDefault();
	                console.log(this.query);
	                const engine = globals.gEngines.get('0');
	                if (!engine)
	                    return;
	...
	unwrapExports(query_page);
	var query_page_1 = query_page.QueryPage;

	var scrollable_container = createCommonjsModule(function (module, exports) {
	// Copyright (C) 2018 The Android Open Source Project
	//
	// Licensed under the Apache License, Version 2.0 (the "License");
	// you may not use this file except in compliance with the License.
	...
	exports.ScrollableContainer = {
	    view({ attrs, children }) {
	        return mithril('.scrollableContainer', {
	            style: {
	        ...
	        }, mithril(ScrollableContent, { contentHeight: attrs.contentHeight }, children));
	    },
	    oncreate({ dom, attrs }) {
	...
	const ScrollableContent = {
	    view({ attrs, children }) {
	        return mithril('.scrollableContent', {
	            style: {
	...
	unwrapExports(track_shell);
	var track_shell_1 = track_shell.TrackShell;
	...
	exports.Frontend = {
	    ...
	    view({}) {
	        ...
	        }, mithril(canvas_wrapper.CanvasWrapper, {
	...
	exports.FrontendPage = pages.createPage({
	    view() {
	        return mithril(exports.Frontend, { width: 1000, height: 300 });
	    }
	});
	...
	function main() {
	    ...
	    mithril.route(root, '/', {
	        '/': home_page.HomePage,
	        '/viewer': exports.FrontendPage,
	        '/query/:trace': query_page.QueryPage,
	    });
	}
	main();
	...
	var index = unwrapExports(frontend);
	var frontend_1 = frontend.Frontend;
	var frontend_2 = frontend.FrontendPage;

	exports.default = index;
	exports.Frontend = frontend_1;
	exports.FrontendPage = frontend_2;
